%PLOTSCENE plot the results
%       Author: Vladislav Pripotnev
%       ID:300331564
%   inputs
%       camOrigin: camera position
%       positionsCars_x: x coordinate of car
%       positionsCars_y: y coordinate of car
%       positionsCars_z: z coordinate of car
%   outputs
%       N/A
function [noOut] = plotScene(camOrigin, ...
    positionsCars_x, positionsCars_y, positionsCars_z)
    ...
    startPnt = 1;
    maxCars = size(positionsCars_x,1);
    maxPnts = size(positionsCars_x,2);
    ...
    for cars=1:maxCars
        for pnts=startPnt:(startPnt+maxPnts-1)
            if positionsCars_x(cars,pnts) ~= -1
